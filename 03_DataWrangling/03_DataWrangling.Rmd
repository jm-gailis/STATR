...
- N'utilisez pas d'accent ou d'espaces dans vos noms de dossiers !
...
- Régle d'or : sur R, pas besoin d'utiliser sa souris. Moins on utilise le "cliq", mieux c'est.
...
Ici, 4 départements, donc un tableau de 4 lignes.
...
3. En les convertissant au format _character_ grâce à la fonction _as.character()_ :

```{r, echo = T, eval = T}
soc_eco$INSEE_COM <- as.character(soc_eco$CODGEO)
glimpse(soc_eco$INSEE_COM) # On a bien un vecteur de type character
```

---
## Réaliser une jointure : les fonctions _join_

Il y a plusieurs types de jointures : 
...
La fonction _left_join_ garde le premier objet et y "colle" les variables du 2e objet. 
...
C'est l'objet au format SF qui nous intéresse : il comporte les données géométriques qui nous permettent de cartographier nos données. 
...
colnames(idf_sf) #Vérifier si les variables de soc_eco ont été ajoutées
```
...
Ces deux lignes de code produisent le même résultat : créer une nouvelle variable à partir d'une variable existante, en passant du format _integer_ au format _character_
...
---
# Exercice à rendre avant le 01/11 

- Calculez le nombre total de meurtres par grande région des USA avec le fichier `murders.csv`

- Calculez et visualisez la densité de  population par département en 2008 avec le fichier `parispc_com.shp`


---
# Exercice à rendre avant le le 01/11 

Rédigez et envoyez un script où vous réalisez, en les commentant, chacune des tâches suivantes :
...
7. Quel est l'arrondissement de Paris qui accueille le plus de tournages en 2016 ? En 2019 ? NB. Il vous sfaut utilier la fonction _group_by_ en utilisant 2 variables...
8. Quel est l'arrondissement de Paris qui accueille le plus de tournages de long métrage en 2018 ? 
9. Quel est l'arrondissement où se déroule le plus de tournage de la série "Emily in Paris" ?
10. Proposez une visualiation des lieux de tournage de 8e saison de la série Engrenages. Quels types de localisations et quels arrondissements sont le plus associés à la série ? 
11. Proposez une visualisation des lieux de tournages du film "120 battements par minute". 

Toutes les fonctions et packages vus en cours vous permettent de répondre à ces questions. 
