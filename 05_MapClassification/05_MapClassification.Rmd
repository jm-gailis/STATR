...
- La variance est la somme du carré des écarts à la moyenne rapporté à l’effectif
...
- Il s'exprime dans l'uniité de mesure de la variable étudiée.
...
- plus la variance ou l'écart-type sont élevés, plus les valeurs sont dispersées autour de la moyenne. 
- plus ils sont faibles et plus les valeurs sont regroupées.

---
### Les quantiles

- Une autre manière de mesurer la dispersion est de calculer les quantiles.
- On appelle quantiles les valeurs du caractère qui définissent les bornes d'une partition en classes d'effectifs égaux.
- Deux types de quantiles sont communément utilisés : 
  ...
  - Les **quartiles** sont les trois valeurs qui permettent de découper la distribution en quatre classes d'effectifs égaux. 
      - Q1 : 1er quartile est la valeur pour laquelle on a 25% des observations en dessous et 75% au dessus. 
      - Q2 : 2e quartile est la valeur pour laquelle on a 50% des observations en dessous et 50% au dessus.
      - Q3:  3e quartile est la valeur pour laquelle on a 75% des observations en dessous et 25% au dessus

  - Les **déciles** sont les neufs valeurs de X qui permettent de découper la distribution en dix classes. 

- L'écart inter-quartile : l'intervalle de valeurs existant entre Q1 et Q3. IQR s'exprime dans l'unité de mesure de la variable. Il dépend du *rang* des observations, et non des valeurs elles-mêmes.

---
### Les quantiles dans R 


La fonction *quantile()* permets de calculer les quantiles, en modifiant l'argument *prob.*

```{r, eval = FALSE}
#pour obtenir la valeur du 1er quartile
quantile(data.frame$Variable, prob = 0.25) 
#pour obtenir la valeur des quartiles
quantile(data.frame$Variable, prob = seq(0, 1, 0.25)) 
```

--

La fonction *summary()* permet d'obtenir ces mesures. 
...
Il permet de visualiser la forme d'une distribution statistique.

La forme de l’histogramme dépend de l’amplitude de classe retenue.
...
- Discrétiser revient à opter pour un compromis.
  ...
  - Résumer au mieux la distribution (conserver l’hétérogénéité) = *le plus grand nombre de classes*.
  - Construire une carte efficace (synthétiser) = *un faible nombre de classes*.
  - Choisir des combinaison de variables visuelles pour restituer les spécificités de la distribution statistique = *sémiologie graphique*.

- Réduire l'information pour cartographier comporte des enjeux :

  - Conserver l’ordre de grandeur ;
  - Conserver la forme de la distribution ;
  - Conserver la dispersion ;
  - Décider de conserver ou non les singularités (les unités spatiales aux valeurs extrêmes).
...
Sur un même script, en utilisant à chaque fois la statistique univariée et la cartographie :

- Proposer une analyse de la distribution des cadres, des ouvriers, du taux de chômage et du revenu médian en 2007 dans la métropole parisienne.
- Commenter l'évolution entre 1999 et 2007 de la population des cadres et des ouvriers.
- Commenter l'évolution de la densité entre 1936 et 2008.
